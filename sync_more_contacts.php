<?php
// Script para sincronizar más contactos desde webhook events
// Ejecutar: https://camcam.com.ve/evoappws/sync_more_contacts.php
...
try {
    ...
    echo "<h2>🔄 Sincronizar Más Contactos y Nombres</h2>";
    
    // 1. Extraer TODOS los contactos desde webhook events (no solo los últimos 100)
    ...
    // 7. Chats sin nombres para diagnóstico
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM chats WHERE name IS NULL OR name = ''");
    $chatsWithoutNames = $stmt->fetch(PDO::FETCH_ASSOC)['total'];
    
    if ($chatsWithoutNames > 0) {
        echo "<h4>⚠️ Chats sin Nombres ($chatsWithoutNames):</h4>";
        $stmt = $pdo->query("SELECT id, remote_jid, is_group FROM chats WHERE name IS NULL OR name = '' ORDER BY id LIMIT 10");
        $chats = $stmt->fetchAll(PDO::FETCH_ASSOC);
        
        echo "<table border='1' style='border-collapse: collapse; width: 100%;'>";
        echo "<tr><th>ID</th><th>Remote JID</th><th>Group</th></tr>";
        foreach ($chats as $chat) {
            echo "<tr>";
            echo "<td>{$chat['id']}</td>";
            echo "<td>{$chat['remote_jid']}</td>";
    ...
    echo "<li>✅ Chats sin nombres: $chatsWithoutNames</li>";
    echo "</ul>";
