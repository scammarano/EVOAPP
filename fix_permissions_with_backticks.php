<?php
/**
 * Fix Permissions Using Backticks for KEY column
 */
...
try {
    ...
    // Insert permissions using backticks around `key`
    $permissions = [
    ...
    $stmt = $pdo->query("SELECT `key`, description FROM permissions ORDER BY `key`");
    $permissions = $stmt->fetchAll();
    
    foreach ($permissions as $permission) {
        echo "- {$permission['key']}: {$permission['description']}<br>";
    }
