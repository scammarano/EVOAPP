<?php
// Script corregido para extraer datos desde API Evolution - Schema Compatible
// Ejecutar: https://camcam.com.ve/evoappws/extract_from_api_fixed.php
...
try {
    ...
    echo "<h2>📡 Extracción Directa desde API Evolution (Schema Compatible)</h2>";
    
    // 1. Verificar schema de tabla contacts
    echo "<h3>🔍 Verificando Schema de Contacts</h3>";
    
    $stmt = $pdo->query("DESCRIBE contacts");
    $columns = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $columnNames = array_map(function($col) { return $col['Field']; }, $columns);
    
    echo "<p>📊 Columnas en contacts: " . implode(', ', $columnNames) . "</p>";
    
    // Verificar si existe la columna status
    $hasStatusColumn = in_array('status', $columnNames);
    
    if (!$hasStatusColumn) {
        echo "<p style='color: orange;'>⚠️ Columna 'status' no existe en contacts</p>";
    }
    ...
    // 3. Extraer todos los chats desde la API
    ...
    // 4. Extraer contactos desde los chats
    echo "<h3>📇 Extrayendo Contactos desde Chats</h3>";
    ...
    foreach ($chats as $chat) {
        ...
        // Saltar grupos para extraer contactos individuales
