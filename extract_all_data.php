<?php
// Script completo para extraer TODOS los datos de contactos, grupos y participantes
// Ejecutar: https://camcam.com.ve/evoappws/extract_all_data.php
...
try {
    ...
    echo "<h2>📊 Extracción COMPLETA de Datos</h2>";
    
    // 1. Extraer TODOS los contactos de TODOS los webhooks
    echo "<h3>📇 Extrayendo TODOS los Contactos (Sin Límite)</h3>";
    
    $contactsInserted = 0;
    $contactsUpdated = 0;
    $totalContactEvents = 0;
    
    // Procesar en lotes grandes para no agotar memoria
    $offset = 0;
    $batchSize = 100;
    ...
    do {
        ...
        if (empty($contactEvents)) break;
        
        echo "<p>📊 Procesando lote " . ($offset / $batchSize + 1) . " (" . count($contactEvents) . " eventos)...</p>";
                ...
                foreach ($contactsList as $contact) {
                    if (isset($contact['remoteJid'])) {
                        ...
                        if (!$existing) {
                            ...
                            $stmt = $pdo->prepare("
                                INSERT INTO contacts (instance_id, phone_e164, name, push_name, remote_jid, profile_pic_url, created_at) 
                                VALUES (?, ?, ?, ?, ?, ?, ?)
                            ");
                            $stmt->execute([
                                $event['instance_id'],
    ...
    } while (count($contactEvents) == $batchSize && $totalContactEvents < 10000); // Límite de seguridad
    ...
    // 2. Extraer TODOS los chats y grupos
    ...
    do {
        ...
        if (empty($chatEvents)) break;
        
        echo "<p>📊 Procesando lote de chats " . ($offset / $batchSize + 1) . " (" . count($chatEvents) . " eventos)...</p>";
    ...
    } while (count($chatEvents) == $batchSize && $totalChatEvents < 10000);
    ...
    // 3. Extraer participantes de grupos (si hay datos de participantes)
    ...
    // 4. Sincronizar nombres entre contacts y chats
    ...
    // 5. Mostrar resultados finales completos
