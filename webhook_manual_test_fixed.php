<?php
// Test Manual COMPLETO de Webhook - Versión CORREGIDA con instance en URL
?>
<!DOCTYPE html>
<html>
<head>
    <title>Webhook Manual Test - Fixed Version</title>
    <style>
        ...
        .fix-notice { background: #d4edda; padding: 15px; border-radius: 5px; border-left: 4px solid #28a745; margin: 20px 0; }
    </style>
</head>
<body>
    <div class="container">
        <div class="header">
            <h1>🚀 Webhook Manual Test - FIXED VERSION</h1>
            <p>Test manual de TODOS los 26 eventos Evolution API con instance en URL (CORREGIDO)</p>
        </div>
        
        <div class="fix-notice">
            <h3>✅ VERSIÓN CORREGIDA</h3>
            <p><strong>Problema solucionado:</strong> Ahora envía el parámetro <code>instance</code> en la URL como espera el webhook.</p>
            <p><strong>Cambio:</strong> <code>fetch('webhook_test_simple.php?instance=' + instance)</code> en lugar de enviar instance en el JSON.</p>
        </div>
    ...
    <script>
        ...
        async function testEvent(eventName) {
            ...
            try {
                // 🔧 CORRECCIÓN CLAVE: Enviar instance en la URL, no en el JSON
                const response = await fetch('webhook_test_simple.php?instance=' + instance, {
                    method: 'POST',
                    headers: {
        ...
        function createPayloadForEvent(eventName, instance) {
            ...
            const basePayload = {
                ...
                // 🔧 Ya no se envía instance aquí, va en la URL
            };
