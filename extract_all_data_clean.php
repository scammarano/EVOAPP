<?php
// Script LIMPIO para extraer TODOS los datos - Sin Duplicados
// Ejecutar: https://camcam.com.ve/evoappws/extract_all_data_clean.php
...
try {
    ...
    echo "<h2>📊 Extracción COMPLETA de Datos (Sin Duplicados)</h2>";
    
    // 1. Limpiar datos problemáticos primero
    echo "<h3>🧹 Limpieza de Datos Problemáticos</h3>";
    
    // Limpiar chats con remote_jid vacío o nulo
    $stmt = $pdo->query("DELETE FROM chats WHERE remote_jid IS NULL OR remote_jid = ''");
    $deletedEmpty = $stmt->rowCount();
    
    if ($deletedEmpty > 0) {
        echo "<p style='color: orange;'>⚠️ Eliminados {$deletedEmpty} chats con remote_jid vacío</p>";
    }
    
    // Verificar constraints existentes
    $stmt = $pdo->query("SHOW INDEX FROM chats WHERE Key_name = 'instance_remote'");
    $hasInstanceRemote = $stmt->rowCount() > 0;
    
    if ($hasInstanceRemote) {
        echo "<p style='color: blue;'>ℹ️ Constraint instance_remote existe en chats</p>";
    }
    
    // 2. Extraer TODOS los contactos de TODOS los webhooks
    echo "<h3>📇 Extrayendo TODOS los Contactos (Limpio)</h3>";
    ...
    // 3. Extraer TODOS los chats y grupos - CON VALIDACIÓN
    echo "<h3>💬 Extrayendo TODOS los Chats y Grupos (Validado)</h3>";
        ...
        if (isset($payload['data'])) {
            ...
            foreach ($chatsList as $chat) {
                if (isset($chat['id'])) {
                    ...
                    // VALIDACIÓN: Saltar si remote_jid está vacío
                    if (empty($remoteJid)) {
                        $chatsSkipped++;
                        continue;
                    }
                    ...
                    if (!$existing) {
                        try {
                            // Insertar nuevo chat - CON VALIDACIÓN
                            $stmt = $pdo->prepare("
                            ...
                            // Si hay error de duplicado, actualizar en lugar de insertar
                            if (strpos($e->getMessage(), 'Duplicate entry') !== false) {
                                ...
                                echo "<p style='color: orange;'>⚠️ Error insertando chat {$chatId}: " . $e->getMessage() . "</p>";
                            }
                        }
                    } else {
                        // Actualizar si hay datos nuevos
